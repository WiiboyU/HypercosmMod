...
{
    public class StarRaiderHead : ModNPC
    {
        ...
        {
            npc.scale = 1f;
            npc.lifeMax = 218;        //this is the npc health
            npc.damage = 23;    //this is the npc damage
            npc.defense = 8;         //this is the npc defense
            npc.knockBackResist = 0f;
            npc.width = 26; //this is where you put the npc sprite width.     important
            npc.height = 34; //this is where you put the npc sprite height.   important
            npc.lavaImmune = true;       //this make the npc immune to lava
            npc.aiStyle = 6;
            aiType = NPCID.StardustWormHead;
            npc.noGravity = true;           //this make the npc float
            npc.noTileCollide = true;        //this make the npc go thru walls
            npc.HitSound = SoundID.NPCHit4;
            npc.DeathSound = SoundID.NPCDeath14;
            banner = npc.type;
            bannerItem = mod.ItemType("StarRaiderBanner");
        ...
        public override void AI()
        {
            ...
            if (npc.ai[1] >= 180)
            {
                ...
                int damage = 25;
                int type = mod.ProjectileType("SmallStarComet");
                ...
                if (Main.netMode != 1)
                {
                    ...
                    if (npc.localAI[0] >= (float)Main.rand.Next(902, 9000))
                    {
                        ...
                        if (Collision.CanHit(npc.position, npc.width, npc.height, player.position, player.width, player.height))
                        {
                            ...
                            int num945 = expertMode ? 65 : 50;
                            int num946 = mod.ProjectileType("SmallStarComet");
            ...
            public override bool PreAI()
        {
            if (Main.netMode != 1)
            {
                ...
                if (npc.ai[0] == 0)
                {
                    ...
                    for (int i = 0; i < randomWormLength; ++i)
                    {
                        ...
                        latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("StarRaiderBody"), npc.whoAmI, 0, latestNPC);
                        Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                        Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
                    }
                    // When we're out of that loop, we want to 'close' the worm with a tail part!
                    latestNPC = NPC.NewNPC((int)npc.Center.X, (int)npc.Center.Y, mod.NPCType("StarRaiderTail"), npc.whoAmI, 0, latestNPC);
                    Main.npc[(int)latestNPC].realLife = npc.whoAmI;
                    Main.npc[(int)latestNPC].ai[3] = npc.whoAmI;
			...
			npc.velocity.X = Utils.Clamp(npc.velocity.X, -speed, speed);
			npc.velocity.Y = Utils.Clamp(npc.velocity.Y, -speed, speed);
		...
		{
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/StarRaiderHeadGore"), 1f);
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/StarRaiderBodyGore"), 1f);
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/StarRaiderBodyGore"), 1f);
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/StarRaiderBodyGore"), 1f);
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/StarRaiderBodyGore"), 1f);
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/StarRaiderBodyGore"), 1f);
			Gore.NewGore(npc.position, npc.velocity, mod.GetGoreSlot("Gores/StarRaiderTailGore"), 1f);
			return true;
		}
        public override void PostDraw(SpriteBatch spriteBatch, Color lightColor)
        {
            Texture2D texture = mod.GetTexture("NPCs/Bosses/StarRaiderHeadGlowmask");
            spriteBatch.Draw
            (
                texture,
                new Vector2
